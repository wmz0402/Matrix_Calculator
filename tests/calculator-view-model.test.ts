...
describe("calculator view model", () => {
  it("formats a scalar determinant result", () => {
    ...
    expect(result).not.toHaveProperty("matrices");
  });
  ...
  it("keeps inverse trace data separate from presentation", () => {
    ...
    expect(result.formula).toContain("A^{-1}");
  ...
  it("formats PLU outputs as a readable multi-line formula", () => {
    const result = calculate({ operation: "plu", leftText: "2 1; 4 3", rightText: "", exponentText: "2" });
    expect(result.formula).toContain("\\begin{aligned}");
    expect(result.formula).toContain("PA&=LU");
  });
